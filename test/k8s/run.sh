...
restarted_pod="${pods[0]}"
stop_port_forwards
kubectl -n "$NS" delete pod "$restarted_pod" --wait=false
kubectl -n "$NS" wait --for=delete "pod/${restarted_pod}" --timeout=120s
kubectl -n "$NS" rollout status deploy/"$APP" --timeout=120s
wait_ready_pods
select_pods
start_port_forwards
...
go run ./test/k8s/client --pod-a "http://127.0.0.1:${PORT_A}" --pod-b "http://127.0.0.1:${PORT_B}"
echo "post-restart k8s socket.io cluster checks passed"
